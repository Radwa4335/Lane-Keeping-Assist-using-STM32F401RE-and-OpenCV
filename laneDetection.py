...
stm32_port = "COM6"  # Replace COMX with your STM32 port, e.g., COM3 or /dev/ttyUSB0
baud_rate = 115200     # Match the STM32 baud rate
...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.path.join(CWD_PATH, '5.mp4'))
...
def offCenter(meanPts, inpFrame):
    ...
    Motor_Order = 1 if deviation < 0 else 2 if deviation > 0 else 3 ##1 means go right ,, 2 means go left ,, 3 means go straight 
    return deviation, direction, Motor_Order
################################################################################
#### END - FUNCTION TO CALCULATE DEVIATION FROM LANE CENTER ####################

def send_float(ser, float_value):
    # Format the float to a string with desired precision
    formatted_value = f"{float_value:.2f}\n"
    
    # Send the formatted value as bytes
    ser.write(formatted_value.encode())  # Send only the number
    
    # Print the value being sent without showing b'' formatting
    print(f"Data sent: {formatted_value}")
...
while True:
    ...
    deviation, directionDev, x = offCenter(meanPts, frame)
    Motor_Order= [0]
    Motor_Order[0] = x
    
    deviation_sent = deviation
    # Initialize the serial connection
    ser = serial.Serial(stm32_port, baud_rate, timeout=1)
    #time.sleep(1)  # Give the connection some time to initialize

    try:
        # Convert the data to bytes and send
        #ser.write(f"{deviation_sent:.2f}\n".encode())
        #print("Data sent:",f"{deviation_sent:.2f}\n".encode())

        send_float(ser, deviation)

        # Optionally, read response from STM32
        response = ser.read(1)  # Read 5 bytes
        print("Response from STM32:", list(response))
